...
# 
# data.m1 %>% 
#   ggplot(aes(x=purchase.date,y=cbm))+
...
MannKendall(cbm_monthly)$sl
notrend_test(cbm_monthly,test = "MK", B = 1e4)$p.value
#pvalor: 0
#Se rechaza H0, si hay tendencia monotona

# Prueba de Cox Stuart ----------------------------------------------------

cbm_monthly %>% ts %>% cs.test
#p-value = 0.003892
#Se rechaza H0, si hay tendencia monotoma
cbm_monthly %>% ts %>% cox.stuart.test
#p-value = 0.000145
#Se rechaza H0, si hay tendencia monotoma
cbm_monthly %>% ts %>% cox.stuart.test(alternative = "left.sided")#decreciente
#p-value = 1
#No se rechaza H0, no tiene tendencia descendiente
cbm_monthly %>% ts %>% cox.stuart.test(alternative = "right.sided")#creciente
#pvalor:  7.248e-05
#Se rechaza H0, hay tendencia monotoma y es creciente o positiva

# Prueba de WAK -----------------------------------------------------------

notrend_test(cbm_monthly, test = "WAVK")$p.value
#pvalor:  0 
...
data.m1 %>% 
  group_by(AnioMes) %>% 
  summarise(IM = mean(cbm)) -> cbm_AnioMes
...
tseries::adf.test(cbm_monthly)
#p-value = 0.4893
#No se rechaza H0, la serie no es estacionaria
aTSA::stationary.test(cbm_monthly,method = "adf")
#No se rechaza H0, la serie no es estacionaria


# Prueba Box Cox ----------------------------------------------------------

(BoxCox.lambda(cbm_monthly) -> l_cbm_monthly)
par(mfrow=c(1,2))
plot(qcbm_monthly,type="l")
plot(BoxCox(cbm_monthly,l_cbm_monthly),type="l")

# Prueba McLeod Li --------------------------------------------------------

McLeod.Li.test(y=cbm_monthly)

# Prueba McLeod Li --------------------------------------------------------

archTest(cbm_monthly, lag=10)

# Modelado ----------------------------------------------------------------

# Media móvil  ------------------------------------------------------------

ts_cbm %>% movavg(n = 3, type="s")#media movil de orden 3

# Data split

ntotal = length(ts_cbm)  # longitud de la serie
ntotal
ntrain = 30 # longitud data de entrenamiento
nvalid = 3  # longitud data de validación
ntest  = 3  # longitud data de prueba

train  = ts_cbm %>% window(start = 1, end = 1+(ntrain-1)/12)
valid  = ts_cbm %>% window(start = 1+(ntrain)/12, end = 1+(ntrain+nvalid-1)/12)
test   = ts_cbm %>% window(start = 1+(ntrain+nvalid)/12)
...
ts_cbm %>% aTSA::stationary.test(method = "kpss",lag.short=T)
ts_cbm %>% aTSA::stationary.test(method = "kpss",lag.short=F)
ts_cbm %>% aTSA::stationary.test(method = "adf")
ts_cbm %>% aTSA::stationary.test(method = "pp",lag.short=F)
ts_cbm %>% aTSA::stationary.test(method = "pp",lag.short=T)
ts_cbm %>% BoxCox.lambda()#aplicar la transformacion
ts_cbm %>% archTest()
ts_cbm %>% McLeod.Li.test(y=.)



ts_cbm %>% diff %>% aTSA::stationary.test(method = "kpss",lag.short=T)
ts_cbm %>% diff %>% aTSA::stationary.test(method = "kpss",lag.short=F)
ts_cbm %>% diff %>% aTSA::stationary.test(method = "adf")
ts_cbm %>% diff %>% aTSA::stationary.test(method = "pp",lag.short=F)
ts_cbm %>% diff %>% aTSA::stationary.test(method = "pp",lag.short=T)
ts_cbm %>% diff %>% BoxCox.lambda()#aplicar la transformacion dentro de argumento transformacion logaritmica
ts_cbm %>% diff %>% archTest()
ts_cbm %>% diff %>% McLeod.Li.test(y=.)

ts_cbm %>% diff %>% TSA::acf(type = "correlation", lag = 28)#extinción
ts_cbm %>% diff %>% TSA::acf(type = "partial", lag = 28)#truncamiento

# Modelamiento ------------------------------------------------------------

ts_cbm %>% auto.arima#evalua automatica p y q

ntotal = length(ts_cbm)
ntrain = 36
...
ts_cbm %>% Arima(order = c(1,1,1)) -> modelo_1
ts_cbm %>% Arima(order = c(0,1,1)) -> modelo_2
...
modelo_1 %>%
  ...
  labs(title = "Residuales del modelo ARIMA(1,1,1)", x = "") + 
...
modelo_1 %>% residuals %>% t.test#media igual a 0
modelo_2 %>% residuals %>% t.test#media igual a 0

modelo_1 %>% residuals %>% shapiro.test #no normalidad
...
residuales_1 %>% TSA::acf(lag=12)#se visualizan 1 autocorrelacion que sale de las bandas pero tiene un desfaso corto al inicio
...
modelo_2 %>% forecast::forecast(h=6)

modelo_2 %>% forecast::forecast(h=6) %>% autoplot +
  labs(x = "Día",
       y = "Y") + 
  theme_minimal()
